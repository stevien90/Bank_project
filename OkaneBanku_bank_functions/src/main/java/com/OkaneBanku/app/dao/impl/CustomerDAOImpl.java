...
public class CustomerDAOImpl implements CustomerDAO {
	...
	public double viewSavingAcc(int customerid) throws BusinessException {
		...
		try {
			...
			String sql = "select balance from bank_schema.saving where savingid = (select savingid from bank_schema.account where customerid = " + customerid + ")";
	...
	public double viewCheckingAcc(int customerid) throws BusinessException { // can use this for validations 
		...
		try {
			Connection connection = pc.getConnection();
			String sql = "select balance from bank_schema.checking where checkingid = (select checkingid from bank_schema.account where customerid = " + customerid + ")";
			Statement statement = connection.createStatement();
			ResultSet rs = statement.executeQuery(sql);
			while(rs.next()) {
				balance = rs.getDouble("balance");
	...
	public double withdrawChecking(int customerid, double withdraw) throws BusinessException {
		...
		try {
			Connection connection = pc.getConnection();
			String sql = "update bank_schema.checking set balance = ? where checkingid =(select checkingid from bank_schema.account where customerid = " + customerid + ")";
	...
	public double withdrawSaving(int customerid, double withdraw) throws BusinessException {
		...
		try {
			Connection connection = pc.getConnection();
			String sql = "update bank_schema.saving set balance = ? where savingid =(select savingid from bank_schema.account where customerid = " + customerid + ")";
			PreparedStatement preparestatement = connection.prepareStatement(sql);
	...
	public double transferChecking(int customerid, double transfer) throws BusinessException {
		...
		try {
			...
			c = preparestatement.executeUpdate();
			String x = "update bank_schema.saving set balance = ? where savingid =(select savingid from bank_schema.account where customerid = " + customerid + ")";
	...
	public double transferSaving(int customerid, double transfer) throws BusinessException {
		...
		try {
			...
			preparestatement.setDouble(1, (savingbalance - transfer));
			c = preparestatement.executeUpdate();
			String x = "update bank_schema.checking set balance = ? where checkingid =(select checkingid from bank_schema.account where customerid = " + customerid + ")";
			preparestatement.setDouble(1, (checkingbalance + transfer));
			c = preparestatement.executeUpdate();
		} catch (ClassNotFoundException | SQLException e) {			
	...
	public void createAccountBalance(double checking, double saving, int customerid) throws BusinessException {
		...
		try {
			Connection connection = pc.getConnection();
			String x = "insert bank_schema.account(customerid, checkingbalance, savingbalance) values (???)";
